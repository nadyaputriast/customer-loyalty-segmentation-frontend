        ...
        <div className="flex flex-col sm:flex-row justify-between gap-4 pt-4">
          ...
          <div className="flex gap-2">
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              <SelectTrigger className="h-9 rounded-full text-xs border-zinc-200 text-zinc-600 bg-zinc-50 hover:bg-zinc-100 w-[180px]">
            ...
            <Button
              variant="outline"
              size="sm"
              className="rounded-full h-9 text-xs border-zinc-200 text-zinc-600 bg-zinc-50 hover:bg-zinc-100"
            >
            ...
            <Button
              variant="outline"
              size="sm"
              className="rounded-full h-9 text-xs font-medium border-zinc-200 text-zinc-600"
            >
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-zinc-100 hover:bg-transparent">
              <TableHead className="w-[300px] h-10 text-xs font-medium text-zinc-500">
                Customer
              </TableHead>
            ...
            {customers.map((customer) => (
              <TableRow key={customer.id} className="border-zinc-100">
                ...
                <TableCell className="py-3">
                  <Badge
                    variant="secondary"
                    className="rounded-full px-2.5 py-0.5 text-[11px] font-normal bg-zinc-100 text-zinc-600 border-0"
                  >
                    {customer.status}
                  </Badge>
                </TableCell>
                <TableCell className="py-3">
                  <div className="flex items-center gap-1.5">
                    {customer.billing === "Paid" ? (
                      <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                    ) : customer.billing === "Overdue" ? (
                ...
                <TableCell className="py-3">
                  <div className="flex flex-col">
                    <span className="text-xs text-zinc-700">
                      {customer.joinedDate.split(" at ")[0]}
                    </span>
                    <span className="text-[11px] text-muted-foreground">
                      at {customer.joinedDate.split(" at ")[1] || "10:00 AM"}
                    </span>
                  </div>
                </TableCell>
              </TableRow>
