...
export default function ClusterScatterPlot({
  ...
  const viewConfig = {
    rvf: { x: "recency", y: "frequency", z: "monetary", xName: "Recency", yName: "Frequency", zName: "Monetary", xUnit: "d", yUnit: "x" },
    rvm: { x: "recency", y: "monetary", z: "frequency", xName: "Recency", yName: "Monetary", zName: "Frequency", xUnit: "d", yUnit: "$" },
    fvm: { x: "frequency", y: "monetary", z: "recency", xName: "Frequency", yName: "Monetary", zName: "Recency", xUnit: "x", yUnit: "$" }
  };
      ...
      return (
        <div className="bg-white/95 backdrop-blur-md p-4 border border-zinc-200/80 rounded-2xl shadow-[0_10px_40px_rgb(0,0,0,0.08)] text-xs space-y-2 z-50 min-w-[160px]">
          ...
          <div className="grid grid-cols-2 gap-2 pt-1">
            ...
            {/* Updated Tooltip to Dollar format */}
      ...
      <CardHeader className="pb-2 px-8 pt-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <CardDescription className="text-sm text-zinc-500 mt-1">
            {currentConfig.xName} vs. {currentConfig.yName} mapping. Bubble size correlates to {currentConfig.zName}.
          </CardDescription>
        </div>

        <div className="flex bg-zinc-50 p-1 rounded-full border border-zinc-200/60">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => setActiveView("rvf")}
            className={`rounded-full h-7 text-xs px-3 transition-all ${activeView === "rvf" ? "bg-white shadow-sm text-zinc-900 font-medium" : "text-zinc-500 hover:text-zinc-700"}`}
          >
            R vs F
          </Button>
          <Button
            variant="ghost"
            size="sm"
            onClick={() => setActiveView("rvm")}
            className={`rounded-full h-7 text-xs px-3 transition-all ${activeView === "rvm" ? "bg-white shadow-sm text-zinc-900 font-medium" : "text-zinc-500 hover:text-zinc-700"}`}
          >
            R vs M
          </Button>
          <Button
            variant="ghost"
            size="sm"
            onClick={() => setActiveView("fvm")}
            className={`rounded-full h-7 text-xs px-3 transition-all ${activeView === "fvm" ? "bg-white shadow-sm text-zinc-900 font-medium" : "text-zinc-500 hover:text-zinc-700"}`}
          >
            F vs M
          </Button>
        </div>
      ...
      <CardContent className="flex-1 min-h-[450px] px-4 pb-8 pt-4">
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 20, right: 30, bottom: 20, left: -10 }}>
            ...
            <ZAxis type="number" dataKey={currentConfig.z} range={[40, 400]} name={currentConfig.zName} />
